...
@property (nonatomic, retain) UIView *clipBorderView;
...
- (void)initView {
    ...
    self.clipBorderView = [[UIView alloc] initWithFrame:self.cropFrame];
    self.clipBorderView.layer.borderColor = [UIColor yellowColor].CGColor;
    self.clipBorderView.layer.borderWidth = 1.0f;
    self.clipBorderView.autoresizingMask = UIViewAutoresizingNone;
    [self.view addSubview:self.clipBorderView];
...
- (void)overlayClipping
{
    ...
    CGPathAddRect(path, nil, CGRectMake(0, 0,
                                        self.clipBorderView.frame.origin.x,
                                        self.overlayView.frame.size.height));
    // Right side of the ratio view
    CGPathAddRect(path, nil, CGRectMake(
                                        self.clipBorderView.frame.origin.x + self.clipBorderView.frame.size.width,
                                        0,
                                        self.overlayView.frame.size.width - self.clipBorderView.frame.origin.x - self.clipBorderView.frame.size.width,
                                        self.overlayView.frame.size.height));
    // Top side of the ratio view
    CGPathAddRect(path, nil, CGRectMake(0, 0,
                                        self.overlayView.frame.size.width,
                                        self.clipBorderView.frame.origin.y));
    // Bottom side of the ratio view
    CGPathAddRect(path, nil, CGRectMake(0,
                                        self.clipBorderView.frame.origin.y + self.clipBorderView.frame.size.height,
                                        self.overlayView.frame.size.width,
                                        self.overlayView.frame.size.height - self.clipBorderView.frame.origin.y + self.clipBorderView.frame.size.height));
    maskLayer.path = path;
...
-(UIImage *)getSubImage{
    ...
    CGFloat scaleRatioW = self.latestFrame.size.width / self.originalImage.size.width;
    CGFloat scaleRatioH = self.latestFrame.size.height / self.originalImage.size.height;
    CGFloat x = (squareFrame.origin.x - self.latestFrame.origin.x) / scaleRatioW;
    CGFloat y = (squareFrame.origin.y - self.latestFrame.origin.y) / scaleRatioH;
    CGFloat w = squareFrame.size.width / scaleRatioW;
    CGFloat h = squareFrame.size.height / scaleRatioH;
